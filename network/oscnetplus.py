...
from .Fconv_wnet import Fconv_PCA
...
class OSCNetplus(nn.Module):
    def __init__(self, args):
        super(OSCNetplus, self).__init__()
        ...
        self.cdiv  = args.cdiv  # controlling the update speed for filter
        ...
        self.fcnn = Fconv_PCA(sizeP=args.sizeP, inNum=1, outNum=self.num_M, tranNum=args.num_rot, inP = args.inP, padding=args.padding, ifIni=args.ifini, bias=True, Smooth = True, iniScale = 1.0, cdiv = self.cdiv)
    ...
    def forward(self, CT_ma, LIct, Mask):
        batchSize, h, w = CT_ma.size()[0], CT_ma.size()[2], CT_ma.size()[3]
        ...
        A_hat = input - A.reshape(batchSize, 1, h, w)
        ...
        for i in range(self.iter):
            # M-net
            A_hat = Mask *(input -  X)
            Epsilon = self.etaM_S[i, :] / 10 * F.conv_transpose2d((Mask*A - A_hat).reshape(1,-1,h,w), C /self.cdiv, groups = batchSize, stride=1, padding=4).reshape(batchSize, -1, h, w)
            M = self.proxNet_M_S[i + 1](M - Epsilon)

            # X-net
            A = F.conv2d(M.reshape(1, -1, h,w), C /self.cdiv, groups=batchSize, stride=1, padding=4).reshape(batchSize,1,h,w)
            ListA.append(A)
            X_hat = input - A
